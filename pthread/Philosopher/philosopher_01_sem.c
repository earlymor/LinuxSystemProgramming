...
sem_t chops[NUM_PHILOSOPHERS];  // 使用信号量模拟筷子使用情况
sem_t sem_eater;
...
void* philosopher(void* ph_num) {
    ...
    for (int i = 0; i < MAX; i++) {
        ...
        sem_wait(&sem_eater); // 确保拿起筷子的数量<=4
        sem_wait(&chops[left]);
...
int main() {
    ...
    int ret;
    ret = sem_init(&sem_eater, 0, 4);
    sys_error(ret, "sem_init");

    for (int i = 0; i < NUM_PHILOSOPHERS; i++) {  // 初始化信号量,即筷子使用情况
        ret = sem_init(&chops[i], 0, 1);
        sys_error(ret, "sem_init");
    }
    ...
    for (int i = 0; i < NUM_PHILOSOPHERS; i++) {  // 销毁信号量
        ret = sem_destroy(&chops[i]);
        sys_error(ret, "sem_destroy");

    }
    sem_destroy(&sem_eater);
}